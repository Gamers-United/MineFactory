...
if name and playerplus[name] then
		...
		nfast = nil
		if playerplus[name].nod_stand == "blocks:ice" then
			nfast = true
		end

		-- are we standing on any nodes that slow player down?
		nslow = nil
		if playerplus[name].nod_stand == "blocks:snow"
		or playerplus[name].nod_stand == "blocks:snowblock" then
			nslow = true
		end
...
--[[
		...
		-- am I near a cactus?
		local near = minetest.find_node_near(pos, 1, "deco:cactus")
